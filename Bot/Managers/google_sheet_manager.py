...
class Sheet_Manager:
    ...
    async def writing_data(self, table: Worksheet, *data):
        user_auto_values = table.get_all_values()
        last_record = len(user_auto_values)

        for cell, data in enumerate(data):
            table.update_cell(last_record + 1, cell + 1, data)
        
        # print(await self._print_data_from_sheet(table))

    async def writing_order(self, sheet: Worksheet, order_id: str, user_id: str, user_name: str, phone_number: str, full_name: str, order: str, address: str):
        data = [order_id, user_id, user_name, phone_number, full_name, order, address]
        user_auto_values = sheet.get_all_values()
        last_record = len(user_auto_values)

        for cell, data in enumerate(data):
            sheet.update_cell(last_record + 1, cell + 1, data)
